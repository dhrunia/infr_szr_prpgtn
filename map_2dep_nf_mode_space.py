...
niters = tf.constant(2000, dtype=tf.int32)
...
y_pred = dyn_mdl.simulate(dyn_mdl._nsteps, dyn_mdl._nsubsteps,
                          dyn_mdl._time_step, dyn_mdl._y_init, x0_pred,
...
theta_true = dyn_mdl.inv_transformed_parameters(x0_true,
                                                ...
                                                param_space='mode')
n_sample_aug = 50
obs_data_aug_test = tf.TensorArray(dtype=tf.float32, size=n_sample_aug)
for j in range(n_sample_aug):
    data_noised = slp_true + tf.random.normal(
        shape=slp_true.shape, mean=0, stddev=0.1)
    obs_data_aug_test = obs_data_aug_test.write(j, data_noised)
obs_data_aug_test = obs_data_aug_test.stack()
loss_gt = -1.0 * dyn_mdl.log_prob(
    theta_true[tf.newaxis, :], obs_data_aug_test, param_space='mode')
loss_pred = -1.0 * dyn_mdl.log_prob(
    theta_pred[tf.newaxis, :], obs_data_aug_test, param_space='mode')
