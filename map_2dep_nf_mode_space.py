...
nsubsteps = tf.cast(tf.math.floordiv(
    sampling_period, time_step), dtype=tf.int32)

y_obs = dyn_mdl.simulate(
...
# %%
lib.plots.seeg.plot_slp(
    slp_true.numpy(), save_dir=f"{figs_dir}/ground_truth", fig_name="slp_obs.png"
)
# %%
...
theta_init_val = dyn_mdl.inv_transformed_parameters(
    x0, eps, param_space="mode")
theta = tf.Variable(initial_value=theta_init_val, dtype=tf.float32)
# %%


@tf.function
def get_loss_and_gradients(obs_data, obs_space):
    with tf.GradientTape() as tape:
        loss = -1.0 * dyn_mdl.log_prob(
            theta[tf.newaxis, :], obs_data, param_space="mode", obs_space=obs_space
        )
...
# @tf.function
def train_loop(num_iters, optimizer, obs_data, obs_space):
    ...
    i, loss_at = tf.while_loop(
        cond=cond, body=body, loop_vars=(i, loss_at), parallel_iterations=1
    )
...
losses = train_loop(niters, optimizer, obs_data_aug, obs_space="sensor")
...
np.save(f"{results_dir}/x0_pred_lmax={dyn_mdl.L_MAX_PARAMS}.npy", x0_pred.numpy())
...
# %%
lib.plots.seeg.plot_slp(
    slp_pred.numpy(), save_dir=f"{figs_dir}/infer", fig_name="slp_pred.png"
)
# %%
...
theta_pred = dyn_mdl.inv_transformed_parameters(
    x0_pred, tf.reshape(eps_pred, shape=(1,)), param_space="mode")
theta_true = dyn_mdl.inv_transformed_parameters(
    x0_true, tf.reshape(eps_pred, shape=(1,)), param_space="mode")
loss_gt = -1.0 * dyn_mdl.log_prob(theta_true[tf.newaxis, :],
                                  obs_data_aug, param_space="mode", obs_space="sensor")
loss_pred = -1.0 * dyn_mdl.log_prob(
    theta_pred[tf.newaxis, :], obs_data_aug, param_space="mode", obs_space="sensor")
...
gs_gt = sub_figs[0].add_gridspec(
    2, 3, height_ratios=[0.9, 0.1], width_ratios=[0.49, 0.49, 0.02]
)
ax_gt = {}
...
gs_infr = sub_figs[1].add_gridspec(
    2, 3, height_ratios=[0.9, 0.1], width_ratios=[0.49, 0.49, 0.02]
)
ax_infr = {}
