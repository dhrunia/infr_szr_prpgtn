...
for i,scr in enumerate(engel_scores):
    ...
    p, r = lib.utils.stan.precision_recall(patient_ids[scr], root_dir,
                                           src_thrshld, t_eps,
                                           parcellation='destrieux', outfile_regex="samples_*.csv",
                                           npoints=npoints)
    precision.append(p)
    recall.append(r)
...
plt.savefig(os.path.join(figs_dir, f'precsision_recall_pergroup_teps{t_eps}.png'), dpi=512)
...
os.makedirs(figs_dir, exist_ok=True)
patient_ids = dict()
patient_ids['engel1'] = ['id001_bt', 'id003_mg', 'id004_bj', 'id010_cmn', 'id013_lk',
...
for i, score in enumerate(engel_scores):
    ...
    for subj_id in patient_ids[score]:
        ...
        p, r = lib.utils.stan.precision_recall(
            [subj_id], root_dir, src_thrshld, t_eps, parcellation='destrieux', outfile_regex='samples_*.csv', npoints=npoints)
...
for i, scr in enumerate(engel_scores):
    x_pos = find_xpos(
        delta=0.09, data_arr=precision[scr], bin_center=(3*i + 1))
...
# ax_cdf.legend()
plt.tight_layout()
plt.savefig(os.path.join(figs_dir, f'precision_recall_per_patient_teps{t_eps}.png'), dpi=512)
