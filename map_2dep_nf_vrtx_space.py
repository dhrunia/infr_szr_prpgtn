...
# %%
# x0 = tf.random.uniform(shape=(dyn_mdl.nv.numpy() + dyn_mdl.ns.numpy(),),
#                        minval=dyn_mdl.x0_lb,
#                        maxval=dyn_mdl.x0_ub,
#                        dtype=tf.float32)

# x0 = -3.0 * np.ones(dyn_mdl.nv + dyn_mdl.ns)
# x0[ez_hyp_vrtcs] = -1.2
...
# x0 = x0_true
eps = tf.constant(0.3, shape=(1, ), dtype=tf.float32)
...
def train_loop(num_iters, optimizer, obs_data, obs_space):
    ...
    def body(i, loss_at):
        ...
        # grads = [tf.clip_by_norm(el, 10) for el in grads]
...
x0_prior_mu = tf.constant(x0, dtype=tf.float32) * dyn_mdl.unkown_roi_mask
...
for j in range(n_sample_aug):
    data_noised = slp_true + tf.random.normal(
        shape=slp_true.shape, mean=0, stddev=0.1)
...
# %%
# x0 = dyn_mdl.x0_bounded(theta_true[0:dyn_mdl.nv + dyn_mdl.ns])
# eps = dyn_mdl.eps_bounded(theta_true[dyn_mdl.nv + dyn_mdl.ns])
# y_test = dyn_mdl.simulate(dyn_mdl._nsteps, dyn_mdl._nsubsteps,
...
#     unkown_roi_mask=dyn_mdl.unkown_roi_mask.numpy())
# %%
x0_pred, eps_pred = dyn_mdl.transformed_parameters(theta, param_space='vertex')
...
theta_true = dyn_mdl.inv_transformed_parameters(x0_true,
                                                tf.reshape(eps_pred,
                                                           shape=(1, )),
                                                param_space='vertex')
# theta_true = tf.concat((x0_hat, eps_hat), axis=0)
