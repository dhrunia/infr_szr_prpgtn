...
theta, phi = np.meshgrid(np.linspace(0, 2*np.pi, N_LON), np.arccos(cos_theta.numpy()))
F_theta_phi = pysh.expand.spharm_lm(20, 10, theta, phi, 'ortho', 'real', -1, False)
...
ftp_hat = pysh.expand.MakeGridGLQ(pysh.shio.SHctor(
    flmc_hat), cos_theta.numpy(), lmax=L_MAX, norm=4, csphase=-1)
# %%
