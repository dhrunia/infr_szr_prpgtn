...
# %% [markdown]
#### Define the variational posterior as a MultivariateNormal with diagonal covariance
#%%
nparams = 3*nn + 4
...
var_dist = tfd.MultivariateNormalDiag(
    loc=loc, scale_diag=scale_diag, name="variational posterior")
...
@tf.function
def loss(posterior_approx, batch_size, obs_slp, SC, gain):
    posterior_samples = posterior_approx.sample(batch_size)
...
@tf.function
def get_loss_and_gradients(posterior_approx, batch_size, obs_slp, SC, gain):
    # nsamples = base_dist_samples.shape[0]
    with tf.GradientTape() as tape:
        loss_val = loss(posterior_approx, batch_size, obs_slp, SC, gain)
...
for epoch in range(num_epochs):
    # var_dist_samples = var_dist.sample(batch_size)
    loss_value, grads = get_loss_and_gradients(
        var_dist, batch_size, obs_data['slp'], SC, gain)
    # grads = [tf.divide(el, batch_size) for el in grads]
    grads = [tf.clip_by_norm(el, 1000) for el in grads]
    # tf.print("gradient norm = ", [tf.norm(el) for el in grads], \
    # output_stream="file://debug.log")
    ...
    training_loss.append(loss_value)
    optimizer.apply_gradients(
        zip(grads, var_dist.trainable_variables))
    # if ((epoch+1) % 10 == 0):
    #     print(f'Epoch {epoch + 1}:\n Training Loss: {loss_value}')
...
# %%
save_figs = True
figs_dir = 'results/exp25/figures'
os.makedirs(figs_dir, exist_ok=True)
figname_suffix = '80epochs'
# %%
...
if(save_figs):
    plt.savefig(os.path.join(
        figs_dir, f'inferred_params_woic_{figname_suffix}.svg'))
