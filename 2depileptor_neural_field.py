...
def epileptor2D_nf_ode_fn(t, y, x0, tau):
    ...
    alpha = tf.constant(5.0, dtype=tf.float32)
    theta = tf.constant(-1.0, dtype=tf.float32)
    gamma = tf.constant(1.0, dtype=tf.float32)
    x_lm = tfsht.analys(N_LON, tf.reshape(tf.math.sigmoid(
        alpha*(x - theta)), (N_LAT, N_LON)), glq_wts, P_l_m_costheta)
    x_lm_hat = Dll[:, tf.newaxis] * x_lm
    local_cplng = tf.reshape(tfsht.synth(
        N_LON, x_lm_hat, P_l_m_costheta), [-1])
    dx = 1.0 - tf.math.pow(x, 3) - 2 * tf.math.pow(x, 2) - \
        z + I1 + gamma * local_cplng
...
def dormandprince_integrator(ode_fn, t_init, y_init, solution_times, constants):
    return tfp.math.ode.DormandPrince(atol=1e-6,
                                      rtol=1e-3,
...
x_init_true = tf.constant(-2.0, dtype=tf.float32) * \
    tf.ones(N_LAT*N_LON, dtype=tf.float32)
